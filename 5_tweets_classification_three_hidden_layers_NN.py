...
using a three layers Neural Network:
a first layer or relu neurons,
a second layers of sigmoid neurons, and
finally a third layer of tanh neurons.
...
def main():
    ...
    epsilon=0.017 #regularization lambda
    EPOCHS=20
    number_first_hidden_layer=50 #relu neurons
    number_second_hidden_layer=25 #sigmoid neurons
    number_third_hidden_layer=10 #tanh nurons
    ...
    print("-----------------------------")
    print("METHOD - NEURAL NETWORK with THREE HIDDEN LAYERS")
    print("-----------------------------")


    print("Hello,\nwe will use a Neural Network with three hidden layers\nto classify twitter users\n")
    setpath()
    ...
    model_bnn.add(tf.keras.layers.Dense(number_third_hidden_layer, kernel_regularizer=regularizers.l2(epsilon), bias_regularizer=regularizers.l2(epsilon), activation='tanh'))
    ...
    if os.path.isfile(FOLDER+"/update_3LNN"+str(n_features)+"_"+str(U)+".txt")==True:
        with open(FOLDER+"/update_3LNN"+str(n_features)+"_"+str(U)+".txt", "r") as h:
            update=h.read()
            h.close()
    #f=FOLDER+"/X_train_politic"+update+".npy"
        ...
        with open(FOLDER+"/X_train_politic"+today_string+"_3LNN"+str(D)+"_"+str(U)+".npy",'wb') as f:
            pickle.dump(X_train,f)


        file=FOLDER+"/X_train_politic"+today_string+"_3LNN"+str(D)+"_"+str(U)+".npy"
        with open(file, "wb") as f:
            pickle.dump(X_train,f)

        file=FOLDER+"/Y_train_politic"+today_string+"_3LNN"+str(D)+"_"+str(U)+".npy"
        with open(file, "wb") as f:
            pickle.dump(Y_train,f)

        file=FOLDER+"/X_dev_politic"+today_string+"_3LNN"+str(D)+"_"+str(U)+".npy"
        with open(file, "wb") as f:
            pickle.dump(X_dev,f)

        file=FOLDER+"/Y_dev_politic"+today_string+"_3LNN"+str(D)+"_"+str(U)+".npy"
        with open(file, "wb") as f:
            pickle.dump(Y_dev,f)

        file=FOLDER+"/X_test_politic"+today_string+"_3LNN"+str(D)+"_"+str(U)+".npy"
        with open(file, "wb") as f:
            pickle.dump(X_test,f)

        file=FOLDER+"/Y_test_politic"+today_string+"_3LNN"+str(D)+"_"+str(U)+".npy"
        with open(file, "wb") as f:
            pickle.dump(Y_test,f)

        with open(FOLDER+"/update_3LNN"+str(D)+"_"+str(U)+".txt", "w") as h:
            h.write(today_string+"_3LNN"+str(D)+"_"+str(U))
            h.close()
